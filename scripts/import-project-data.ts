...
function parseRepetition(repetition: string): { type: string; frequency: number; interval: number } {
...
async function importProjectDocuments() {
  ...
  const documents = parseCSV(content) as ProjectDocumentCSV[];
...
async function importProjectTasks() {
  ...
  const content = fs.readFileSync(csvPath, 'utf-8');
  const tasks = parseCSV(content) as ProjectTaskCSV[];
...
export { main as importProjectData };
